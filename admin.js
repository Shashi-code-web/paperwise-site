...
async function authFetch(url,options={}){if(!sb)throw new Error('Supabase browser configuration is missing.');const {data:{session}}=await sb.auth.getSession();if(!session)throw new Error('Please sign in first.');return fetch(url,{...options,headers:{'Content-Type':'application/json',...(options.headers||{}),Authorization:'Bearer '+session.access_token}})}
async function loadAdmin(){
  ...
  (d.products||[]).slice(0,rows.length).forEach((p,i)=>{rows[i].querySelector('b').textContent=p.title;rows[i].querySelectorAll('b')[1].textContent='₹'+(p.price_paise/100);rows[i].querySelector('.status').textContent=p.active?'Active':'Draft';});
}
